...
while IFS= read -r link; do
    ...
    for cache_size in "${relative_cache_sizes[@]}"; do
        ...
        if ! [ -s $datasets ]; then
            echo "shell:1:$min_dram:2:~/OptimalClockOffline/build/cacheSimulator\
              $file -o $basedir/OptimalClockOffline/result \
              -r $cache_size -i $max_iteration \
              --generate-datasets --ignore-obj-size -d ignore_obj_size \
              && ~/OptimalClockOffline/python/.venv/bin/python describe_csv.py $datasets > $datasets.desc" >> ~/task
        else
