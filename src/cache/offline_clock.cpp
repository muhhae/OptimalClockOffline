	...
	while (obj_to_evict->clock.freq >= 1) {
		...
		if (custom_params->generate_datasets) {
			out_dataset(
				...
				data.current_req_metadata.last_access_vtime, data.current_req_metadata.create_rtime,
				data.current_req_metadata.clock_time,
				data.current_req_metadata.clock_time / custom_params->max_clock_time,
				data.current_req_metadata.compulsory_miss, data.current_req_metadata.first_seen,
				cache->cache_size, data.current_req_metadata.obj_size,
