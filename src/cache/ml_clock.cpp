...
void MLClockEvict(cache_t *cache, const request_t *req) {
  ...
  while (obj_to_evict->clock.freq >= 1) {
    ...
    features["clock_time"] =
        static_cast<int64_t>(data.current_req_metadata.clock_time);
    features["time_since"] =
        static_cast<int64_t>(data.current_req_metadata.time_since);
    ...
    features["clock_freq"] =
        static_cast<int64_t>(data.current_req_metadata.access_freq);
